   ...
   public void stateMachine(int state) throws IOException {
      ...
      switch (state) {
         ...
         case 1:     // waiting for HELO
            while (!(fromClient = cSocketIn.readLine()).equals("HELO") || !(fromClient = cSocketIn.readLine()).equals("QUIT")) {
         ...
         case 2:     // waiting for MAIL FROM
            while (!(fromClient = cSocketIn.readLine()).equals("MAIL FROM") || !(fromClient = cSocketIn.readLine()).equals("QUIT")) {
         ...
         case 3:     // waiting for RCPT TO
            while (!(fromClient = cSocketIn.readLine()).equals("RCPT TO") || !(fromClient = cSocketIn.readLine()).equals("QUIT")) {
         ...
         case 4:     // waiting for DATA
            while (!(fromClient = cSocketIn.readLine()).equals("DATA") || !(fromClient = cSocketIn.readLine()).equals("QUIT")) {
